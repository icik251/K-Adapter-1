...
def train(args, train_dataset, model, tokenizer):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    p
                    for n, p in regressor_model.named_parameters()
    ...
    if args.fp16:
        ...
        if args.freeze_bert:
            regressor_model, optimizer = amp.initialize(
                regressor_model, optimizer, opt_level=args.fp16_opt_level
            )
        else:
            regressor_model, optimizer = amp.initialize(
                regressor_model, optimizer, opt_level=args.fp16_opt_level
    ...
    if args.local_rank != -1:
        if args.freeze_bert:
            regressor_model = torch.nn.parallel.DistributedDataParallel(
                regressor_model,
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.fp16:
                ...
                torch.nn.utils.clip_grad_norm_(
                    regressor_model.parameters(), args.max_grad_norm
                )
