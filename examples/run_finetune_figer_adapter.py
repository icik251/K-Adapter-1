...
def train(args, train_dataset, model, tokenizer):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    p
                    for n, p in figer_model.named_parameters()
    ...
    if args.fp16:
        ...
        if args.freeze_bert:
            figer_model, optimizer = amp.initialize(
                figer_model, optimizer, opt_level=args.fp16_opt_level
            )
        else:
            figer_model, optimizer = amp.initialize(
                figer_model, optimizer, opt_level=args.fp16_opt_level
            )
            pretrained_model, optimizer = amp.initialize(
                pretrained_model, optimizer, opt_level=args.fp16_opt_level
            )
    ...
    if args.local_rank != -1:
        if args.freeze_bert:
            figer_model = torch.nn.parallel.DistributedDataParallel(
                figer_model,
    ...
    if args.restore:
        if os.path.exists(os.path.join(args.output_dir, "global_step.bin")):
            ...
            if hasattr(figer_model, "module"):
                figer_model.module.load_state_dict(
                    torch.load(os.path.join(output_dir, "pytorch_task_model.bin"))
                )
            else:
                figer_model.load_state_dict(
                    torch.load(os.path.join(output_dir, "pytorch_task_model.bin"))
                )
    ...
    for epoch in range(start_epoch, int(args.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            if args.fp16:
                ...
                loss.backward()
                torch.nn.utils.clip_grad_norm_(
                    figer_model.parameters(), args.max_grad_norm
                )
                torch.nn.utils.clip_grad_norm_(
                    pretrained_model.parameters(), args.max_grad_norm
                )
    ...
    for dataset_type in ["test"]:
        ...
        for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
            ...
            for batch in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                erine_tmp_eval_accuracy, erine_tmp_pred, erine_tmp_true = accuracy(
                    erine_preds, erine_out_label_ids
                )
            ...
            logger.info(
                "{} result: ".format(dataset_type)
            )  # cnt, acc, loose_micro(y2, y1), loose_macro(y2, y1)
            logger.info(
                "result:{}".format(result)
            )  # cnt, simple acc, loose_micro(y2, y1), loose_macro(y2, y1)
            ...
            if os.path.exists(
                os.path.join(args.output_dir, args.my_model_name + "_result.txt")
            ):
                result_file = open(
                    os.path.join(args.output_dir, args.my_model_name + "_result.txt"),
                    "a",
...
def load_and_cache_examples(args, task, tokenizer, dataset_type, evaluate=False):
    ...
    dataset = TensorDataset(
        all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_start_ids
    )
    return dataset
...
class FIGERModel(nn.Module):
    def __init__(
        self, args, pretrained_model_config, fac_adapter, et_adapter, lin_adapter
    ):
        ...
        if self.args.fusion_mode == "concat":
            self.task_dense_lin = nn.Linear(
                self.config.hidden_size + self.config.hidden_size,
                self.config.hidden_size,
            )
            self.task_dense_fac = nn.Linear(
                self.config.hidden_size + self.config.hidden_size,
                self.config.hidden_size,
            )
            self.task_dense = nn.Linear(
                self.config.hidden_size + self.config.hidden_size,
                self.config.hidden_size,
            )
    ...
    def forward(
        ...
        if self.args.fusion_mode == "add":
            ...
            fac_features = self.task_dense_fac(
                torch.cat([combine_features, fac_adapter_outputs], dim=2)
            )
            lin_features = self.task_dense_lin(
                torch.cat([combine_features, lin_adapter_outputs], dim=2)
            )
            task_features = self.task_dense(
                torch.cat([fac_features, lin_features], dim=2)
            )
...
def main():
    ...
    parser.add_argument(
        "--task_adapter", default="", type=str, help="Whether to use task adapter"
    )
    parser.add_argument(
    ...
    figer_model = FIGERModel(
        args,
        pretrained_model.config,
        fac_adapter=fac_adapter,
        et_adapter=et_adapter,
    ...
    if args.do_train:
        train_dataset = load_and_cache_examples(
            args, args.task_name, tokenizer, dataset_type="train", evaluate=False
        )
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model_to_save = (
            figer_model.module if hasattr(figer_model, "module") else figer_model
        )  # Take care of distributed/parallel training
