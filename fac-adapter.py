    ...
    for epoch in range(start_epoch, int(args.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            if args.fp16:
                ...
                loss.backward()
                torch.nn.utils.clip_grad_norm_(
                    adapter_model.parameters(), args.max_grad_norm
                )
...
def evaluate(args, val_dataset, model, tokenizer):
    ...
    output_eval_file = os.path.join(
        args.output_dir, args.my_model_name + "_eval_results.txt"
    )
...
class AdapterModel(nn.Module):
    ...
    def forward(
        ...
        logits = self.out_proj(
            self.dropout(
                self.dense(
                    torch.cat((subj_output.squeeze(1), obj_output.squeeze(1)), dim=1)
                )
            )
        )
...
def load_and_cache_examples(args, task, tokenizer, dataset_type, evaluate=False):
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features_trex(
            examples,
            label_list,
            args.max_seq_length,
            tokenizer,
    ...
    all_subj_special_start_ids = torch.tensor(
        [f.subj_special_start_id for f in features], dtype=torch.float
    )
    all_obj_special_start_ids = torch.tensor(
        [f.obj_special_start_id for f in features], dtype=torch.float
    )
...
def main():
    ...
    parser.add_argument(
        "--max_seq_length", type=int, default=256, help="max lenght of token sequence"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--meta_adapter_model", type=str, help="the pretrained adapter model"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="how many negative samples to select",
    )
    ...
    val_dataset = load_and_cache_examples(
        args, args.task_name, tokenizer, "dev", evaluate=True
    )
